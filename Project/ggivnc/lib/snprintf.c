...
/* Make the #ifdefs easier to follow... */
...
static void fmtstr(char *buffer, size_t * currlen, size_t maxlen,
		    const char *value, int min, int max, int flags);
...
static void
fmtstr(char *buffer, size_t *currlen, size_t maxlen, const char *value,
	...
	int min, int max, int flags)
{
	...
	fracpart = _round((_pow10(max)) * (ufvalue - intpart));

	if (fracpart >= _pow10(max)) {
		intpart++;
		fracpart -= _pow10(max);
	}
#ifdef DEBUG_SNPRINTF
