...
static void render_texture(int width, int height, Texture *t)
{
        ...
        const char *pos = banner[y];
                ...
                if (sy >= 0) {
                        ...
                        dest = static_cast<unsigned char*>( image_buf );
...
int ggi_main(int argc, char **argv)
{
        int gen_millis;
...
#undef CMPOPT
    ...
    y = screen_width/2;
        ...
        for(;;) {
                ...
                if (ggiEventPoll(vis, gii_event_mask( emKeyPress|emPtrRelative ), &tv)) {

            ggi_event ev;
            int done=0;

            ggiEventRead(vis, &ev, gii_event_mask( emKeyPress|emPtrRelative ) );
